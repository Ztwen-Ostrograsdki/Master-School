...
<div class="p-0 m-0 mt-0 mb-2 row {{$width}}">
    ...
    <input placeholder="{{$placeholder_text}}" class="text-white form-control bg-transparent border border-white px-2 @error('{{$modelName}}') text-danger border-danger @enderror" wire:model.defer="{{$modelName}}" type="{{$type}}" name="{{$modelName}}" id="{{$inputId}}">
    @if($error)
