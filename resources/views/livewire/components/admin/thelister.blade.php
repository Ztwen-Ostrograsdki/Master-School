        ...
        <form x-show="selector" x-transition class="w-100 mx-auto" wire:submit.prevent="submitSelecteds">
            <div class="w-100 d-flex justify-content-between my-2 mb-3">
                ...
                <div class="col-7 d-flex justify-content-end">
                    ...
                    <strong title="Inverser la selection" x-on:click="@this.call('inverseCheckeds')"  class="py-1 px-2 mx-1 text-dark cursor-pointer text-center d-inline-block bg-info border border-white rounded">
                        <span class="fa bi-info-square-fill"></span>
                    </strong>
                    <strong title="Tout cocher" x-on:click="@this.call('checkAll')"  class="py-1 px-2 text-dark cursor-pointer text-center d-inline-block bg-success border border-white rounded">
                        <span class="fa bi-check-all"></span>
                    </strong>
                    <button wire:click="setAction('forceDeleteMass')"  class="py-1 mx-1 px-2 text-dark cursor-pointer text-center d-inline-block bg-danger border border-white rounded">
                        <span class="fa fa-trash"></span>
                    </button>
                    @if($adminTrashedData) 
                    <button wire:click="setAction('restoreMass')"  class="py-1 px-1 mr-1 text-dark cursor-pointer text-center d-inline-block bg-success border border-white rounded">
                        <span class="fa bi-recycle"></span>
                    </button>
                    @else
                    <button wire:click="setAction('deleteMass')"  class="py-1 px-1 mx-1 text-dark cursor-pointer text-center d-inline-block bg-info border border-white rounded">
                        <span class="fa fa-trash"></span>
                    </button>
                    @endif
                    <button wire:click="setAction('resetGaleryMass')"  class="py-1 px-1 text-dark cursor-pointer text-center d-inline-block bg-secondary border border-white rounded">
                        <span class="fa bi-image-alt"></span>
                    </button>
                    <button wire:click="setAction('resetBasketMass')"  class="py-1 px-1 mx-1 text-dark cursor-pointer text-center d-inline-block bg-primary border border-white rounded">
                        <span class="fa bi-cart-x"></span>
                    </button>
                    @if($adminTagName == 'unconfirmed')
                    <button wire:click="setAction('confirmUserEmailMass')"  class="py-1 px-1 text-dark cursor-pointer text-center d-inline-block bg-primary border border-white rounded">
                        <span class="fa bi-envelope-check"></span>
                    </button>
                    @endif
                </div>
            </div>
