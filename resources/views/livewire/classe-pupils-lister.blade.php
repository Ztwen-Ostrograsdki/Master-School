            ...
            <table class="w-100 m-0 p-0 table-striped table-bordered z-table text-white">
                ...
                <tbody>
                    @foreach($pupils as $k => $p)
                            <tr class="@if($p->abandonned) text-white-50 @endif">
                                    ...
                                    <span class="d-flex w-100">
                                        ...
                                        <a class="text-white w-100 m-0 p-0" href="{{route('pupil_profil', ['id' => $p->id])}}">
                                            <span class="d-flex">
                                                ...
                                                <span class="@if($p->abandonned) text-white-50 @endif mx-2 d-none d-lg-inline d-xl-inline text-small @if($p->sexe == 'female') text-orange  @endif ">
                                                    {{$p->getName()}}
                                                    @if($p->abandonned)
                                                        <small class="text-warning font-italic ml-2"> (Cet apprenant a abandonné) </small>
                                                    @endif
                                                </span>
                                            </span>
                                        </a>
                                ...
                                @if(!$p->abandonned)
                                    <td class="text-center">
                                        <span wire:click="toAbandonned({{$p->id}})" title="Marquer l'apprenant {{$p->name}} comme abandon" class="text-secondary m-0 p-0 cursor-pointer">
                                            <span class="fa bi-eye py-2 px-2"></span>
                                        </span>
                                    </td>
                                @else
                                    <td class="text-center">
                                        <span wire:click="toAbandonned({{$p->id}})" title="Retirer l'apprenant {{$p->name}} des abandons" class="text-secondary m-0 p-0 cursor-pointer">
